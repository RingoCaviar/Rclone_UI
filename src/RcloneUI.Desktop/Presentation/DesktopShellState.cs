...
public sealed class DesktopShellState : INotifyPropertyChanged
{
    ...
    public bool IsScheduleJourney => route == "Schedules";
    ...
    public DesktopActionNotificationKind ActionNotificationKind => lastAction switch
    {
        ...
        "remote-input-invalid" or "remote-host-key-required" or "remote-test-failed" or "remote-delete-confirmation-required" or "remote-delete-blocked-profile" or "remote-delete-conflict" or "remote-delete-unavailable" or "remote-delete-invalid" or "folder-create-invalid" or "folder-create-failed" or "folder-create-unavailable" or "file-delete-confirmation-required" or "file-delete-invalid" or "file-delete-failed" or "file-delete-unavailable" or "file-rename-confirmation-required" or "file-rename-invalid" or "file-rename-failed" or "file-rename-unavailable" or "copy-cancel-invalid" or "copy-cancel-not-running" or "copy-cancel-unavailable" or "copy-cancel-failed" or "copy-source-equals-destination" or "vault-locked" or "host-unavailable" or "rclone-unavailable" or "mount-prerequisites-unavailable" or "mount-profile-required" or "mount-profile-input-invalid" or "mount-drain-not-proved" or "source-remote-required" or "destination-remote-required" or "download-folder-required" or "upload-folder-required" or "transfer-limits-invalid" or "shutdown-blocked-active-mount" or "winfsp-installer-unavailable" or "winfsp-installer-not-started" or "winfsp-download-failed" or "winfsp-hash-mismatch" or "winfsp-signature-invalid" or "winfsp-install-cancelled" or "winfsp-uac-cancelled" => DesktopActionNotificationKind.Error,
    ...
    public string TransferReadinessMessage => TransferValidationKey switch
    {
        ...
        "copy-source-equals-destination" => T("来源和目标是同一个 Remote 路径。请选择不同的目标文件夹。", "Source and destination are the same Remote path. Choose a different destination folder."),
        _ => string.Empty
    };
    ...
    public string JourneyDescription => route switch { "Remotes" => T("通过三步向导添加云端账号；凭据只发送给后台服务并写入加密保险库。", "Add cloud accounts in three guided steps. Credentials go only to the Host and encrypted Vault."), "Transfers" => T("下载、上传或远程复制。每次操作都是单向复制，不会删除来源或执行镜像。", "Download, upload, or copy between Remotes. Each action is a one-way copy; it does not delete the source or mirror a destination."), "Mounts" => T("创建稳定盘符的挂载配置。Ready 与安全卸载均需要完整证据。", "Create stable drive profiles. Ready and safe unmount require complete evidence."), "Schedules" => T("计划任务的后台调度基础尚未以可持久化、可编辑的桌面流程开放。", "The scheduling foundation is not yet exposed as a persistent, editable desktop workflow."), "Activity" => T("查看真实结果、部分完成、未知状态和可脱敏导出的诊断信息。", "Review truthful outcomes, partial results, unknown states, and redacted diagnostics."), _ => T("此功能通过后台服务快照和命令工作；界面不直接操作 rclone 或保险库。", "This journey uses Host snapshots and commands; the UI never operates rclone or the Vault directly.") };
    public string JourneyStatus => IsScheduleJourney ? T("暂未开放", "Not available yet") : connection == DesktopConnectionState.ConnectedOperational ? T("可以开始", "Ready") : T("当前不可执行", "Action unavailable");
    public string JourneyActionHint => IsScheduleJourney ? T("为避免创建无法可靠保存或恢复的自动任务，此版本不提供计划任务编辑。", "To avoid creating automatic work that cannot be reliably persisted or recovered, this version does not offer schedule editing.") : connection == DesktopConnectionState.ConnectedOperational ? T("普通用户默认值已启用，高级选项按需展开。", "Ordinary-user defaults are active; advanced options remain available on demand.") : AttentionDetail;
    public bool IsJourneyPrimaryAvailable => !IsScheduleJourney;
    public string JourneyPrimaryAction => route switch { "Remotes" => T("添加远程存储", "Add Remote"), "Browser" => T("读取目录", "List folder"), "Transfers" when TransferMode == DesktopTransferMode.Download => T("开始下载", "Start download"), "Transfers" when TransferMode == DesktopTransferMode.Upload => T("开始上传", "Start upload"), "Transfers" => T("开始复制", "Start copy"), "Mounts" when HasActiveMount => T("安全卸载", "Safe unmount"), "Mounts" when MountNeedsRemount => T("重新挂载", "Remount"), "Mounts" when MountRecoveryRequired => T("需要人工恢复", "Manual recovery required"), "Mounts" when mountCachePreset.Key == "standard-read-write" => T("读写挂载", "Mount read/write"), "Mounts" => T("只读挂载", "Mount read-only"), "Activity" => T("刷新活动", "Refresh activity"), _ => T("继续", "Continue") };
    ...
    public void PrepareBrowserFolderForUpload()
    {
